...
def add_to_order(request: HttpRequest, item_pk: int) -> redirect:
...
def delete_item_from_order(request: HttpRequest, item_pk: int) -> redirect:
    ServiceOrder.delete_item(request, item_pk)
    return redirect('home')


def add_discount(request: HttpRequest) -> redirect:
    create_tax(request)
